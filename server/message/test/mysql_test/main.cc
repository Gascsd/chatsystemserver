...
void insert_test(zht_im::MessageTable &tb)
{
    zht_im::Message m1("消息ID1", "会话ID1", "用户ID1", 0, boost::posix_time::time_from_string("2002-01-20 23:59:59.000"));
    tb.insert(m1);
    zht_im::Message m2("消息ID2", "会话ID1", "用户ID2", 0, boost::posix_time::time_from_string("2002-01-21 23:59:59.000"));
    tb.insert(m2);
    zht_im::Message m3("消息ID3", "会话ID1", "用户ID3", 0, boost::posix_time::time_from_string("2002-01-22 23:59:59.000"));
    tb.insert(m3);

    zht_im::Message m4("消息ID4", "会话ID2", "用户ID4", 0, boost::posix_time::time_from_string("2002-01-20 23:59:59.000"));
    tb.insert(m4);
    zht_im::Message m5("消息ID5", "会话ID2", "用户ID5", 0, boost::posix_time::time_from_string("2002-01-21 23:59:59.000"));
    tb.insert(m5);
}
void remove_test(zht_im::MessageTable &tb)
{
    tb.remove("会话ID2");
}
void recent_test(zht_im::MessageTable &tb)
{
    auto res = tb.recent("会话ID1", 2);
    auto begin = res.rbegin(), end = res.rend();
    for(; begin != end; ++begin)
    {
...
void range_test(zht_im::MessageTable &tb)
{
